...
def add_to_cart(request):
    ...
    if serializer.is_valid():
        ...
        logger.info(
            f"Cart for user {request.user_id}: {'created' if created else 'found'}"
        )
...
def clear_cart(request):
    ...
    try:
        ...
        return Response(
            {"message": "Cart is already empty"}, status=status.HTTP_204_NO_CONTENT
        )
...
def cart_summary(request):
    ...
    try:
        ...
        return Response({"total_items": 0, "total_amount": 0, "items_count": 0})
