...
from rest_framework import transaction
...
def create_order(request):
    ...
    try:
        with transaction.atomic():
            ...
            logger.info(
                f"Creating order for user {user_id}: {len(cart_data['items'])} items"
            )
            ...
            for cart_item in cart_data["items"]:
                items_to_reserve.append({"product_id": cart_item["product_id"], "quantity": cart_item["quantity"]})

            logger.info(f"Reserving items {items_to_reserve}")
            if not ProductService.reserve_products(items_to_reserve):
                return Response({"error": "Failed to reserve products"}, status=400)
